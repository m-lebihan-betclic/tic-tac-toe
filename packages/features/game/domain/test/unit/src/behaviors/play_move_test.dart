...
void main() {
  ...
  given('a square that already holds a mark',
          (_) => gameOf('X../.../...', status: const GameStatus.playerTurn()))
  ...
  given('a game waiting on the CPU',
          (_) => gameOf('X../.../...', status: const GameStatus.cpuTurn()))
      .when('the player takes a free square anyway', (sut, _) => const PlayMove()(sut, 1))
      .then('the move is refused as out of turn',
          (result, _) => result.exceptionOrNull().should.be(const MoveError.notYourTurn()));

  given('a game that is already over', (_) => gameOf(
            'XXX/OO./...',
      ...
      .then('the move is refused because the game is over',
          (result, _) => result.exceptionOrNull().should.be(const MoveError.gameOver()));

  given('two marks in a line with the third square free',
          (_) => gameOf('XX./OO./...', status: const GameStatus.playerTurn()))
      .when('the player takes the third', (sut, _) => const PlayMove()(sut, 2))
      .then('the game is won, carrying the line that won it', (result, _) {
  ...
  given('a board one square from full with no line available',
          (_) => gameOf('XXO/OOX/XO.', status: const GameStatus.playerTurn()))
      .when('the player fills the last square', (sut, _) => const PlayMove()(sut, 8))
      .then('the game is drawn and no line is reported', (result, _) {
