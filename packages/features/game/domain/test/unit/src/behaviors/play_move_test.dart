...
void main() {
  given('an empty board', (_) => Game.initial(difficulty: Difficulty.easy))
      .when('the player takes the centre slot', (sut, _) => const PlayMove()(sut, 4))
  ...
  given('a slot that already holds a mark', (_) => gameOf('X../.../...', status: const GameStatus.playerTurn()))
  ...
  given('a game waiting on the CPU', (_) => gameOf('X../.../...', status: const GameStatus.cpuTurn()))
      .when('the player takes a free slot anyway', (sut, _) => const PlayMove()(sut, 1))
      .then(
  ...
  given(
      ...
      .when('the player takes a free slot', (sut, _) => const PlayMove()(sut, 5))
      .then(
  ...
  given(
    'two marks in a line with the third slot free',
  ...
  given(
    'a board one slot from full with no line available',
    (_) => gameOf('XXO/OOX/XO.', status: const GameStatus.playerTurn()),
  ).when('the player fills the last slot', (sut, _) => const PlayMove()(sut, 8)).then(
