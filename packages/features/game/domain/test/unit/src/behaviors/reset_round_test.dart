...
void main() {
  given('a game in progress on a hard board', (_) => gameOf(
            'XO./.X./...',
            status: const GameStatus.cpuTurn(),
            difficulty: Difficulty.hard,
          ))
      .when('the player resets the round', (sut, _) => const ResetRound()(sut))
      .then('the board is empty, it is the player\'s turn, and the level is unchanged',
  ...
  given('a round being played on easy', (_) => Game.initial(difficulty: Difficulty.easy))
      .when('the next round is started on hard',
          (sut, _) => (sut, const StartRound()(difficulty: Difficulty.hard)))
